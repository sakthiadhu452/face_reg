...
# Set src=0 to use the built-in laptop webcam
vs = VideoStream(src=0, framerate=10).start()  # Use src=0 for built-in webcam
time.sleep(2.0)
...
async def detected():
    url = 'https://face-reg-2.onrender.com/set-true'
        ...
        try:
            async with session.post(url) as response:
                if response.status == 200:
                    result = await response.text()  # or .json() if the response is JSON
                    print(result)
                else:
...
while True:
    ...
    for encoding in encodings:
        ...
        if currentname != name:
            currentname = name
            print("Detected:", currentname)
            if currentname == 'Unknown':
                # Run the async function
                asyncio.run(detected())
